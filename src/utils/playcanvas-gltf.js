...
import Animation from './playcanvas-anim'
...
function getPrimitiveType(primitive) {
    ...
    if (primitive.hasOwnProperty('mode')) {
        switch (primitive.mode) {
            case 0:
                primType = pc.PRIMITIVE_POINTS
                break
            case 1:
                primType = pc.PRIMITIVE_LINES
                break
            case 2:
                primType = pc.PRIMITIVE_LINELOOP
                break
            case 3:
                primType = pc.PRIMITIVE_LINESTRIP
                break
            case 4:
                primType = pc.PRIMITIVE_TRIANGLES
                break
            case 5:
                primType = pc.PRIMITIVE_TRISTRIP
                break
            case 6:
                primType = pc.PRIMITIVE_TRIFAN
                break
        }
    }
...
function getFilter(filter) {
    ...
    switch (filter) {
        case 9728:
            pcFilter = pc.FILTER_NEAREST
            break
        case 9729:
            pcFilter = pc.FILTER_LINEAR
            break
        case 9984:
            pcFilter = pc.FILTER_NEAREST_MIPMAP_NEAREST
            break
        case 9985:
            pcFilter = pc.FILTER_LINEAR_MIPMAP_NEAREST
            break
        case 9986:
            pcFilter = pc.FILTER_NEAREST_MIPMAP_LINEAR
            break
        case 9987:
            pcFilter = pc.FILTER_LINEAR_MIPMAP_LINEAR
            break
    }
...
function getWrap(wrap) {
    ...
    switch (wrap) {
        case 33071:
            pcWrap = pc.ADDRESS_CLAMP_TO_EDGE
            break
        case 33648:
            pcWrap = pc.ADDRESS_MIRRORED_REPEAT
            break
        case 10497:
            pcWrap = pc.ADDRESS_REPEAT
            break
    }
...
function getAccessorData(gltf, accessor, buffers) {
    ...
    var accessorByteOffset = accessor.hasOwnProperty('byteOffset')
        ? accessor.byteOffset
        : 0
    var bufferViewByteOffset = bufferView.hasOwnProperty('byteOffset')
    ...
    switch (accessor.componentType) {
        case 5120:
            data = new Int8Array(arrayBuffer, byteOffset, length)
            break
        case 5121:
            data = new Uint8Array(arrayBuffer, byteOffset, length)
            break
        case 5122:
            data = new Int16Array(arrayBuffer, byteOffset, length)
            break
        case 5123:
            data = new Uint16Array(arrayBuffer, byteOffset, length)
            break
        case 5125:
            data = new Uint32Array(arrayBuffer, byteOffset, length)
            break
        case 5126:
            data = new Float32Array(arrayBuffer, byteOffset, length)
            break
    }
...
function translateAnimation(data, resources) {
    ...
    data.channels.forEach(function (channel) {
        ...
        if (path === 'weights') {
            ...
            if (valuesContainTangents) numCurves /= 3
            ...
            switch (path) {
                case 'translation':
                    ensureKeyType(AnimationKeyableType.VEC)
                ...
                case 'scale':
                    ...
                    break
                case 'rotation':
                    ensureKeyType(AnimationKeyableType.QUAT)
...
function translateImage(data, resources, success) {
                ...
                if (texture.source === imageIndex) {
                    ...
                    if (
                        (!isPowerOf2(image.width) ||
                            !isPowerOf2(image.width)) &&
                        (t.addressU === pc.ADDRESS_REPEAT ||
                            t.addressU === pc.ADDRESS_MIRRORED_REPEAT ||
                            t.addressV === pc.ADDRESS_REPEAT ||
                            t.addressV === pc.ADDRESS_MIRRORED_REPEAT ||
                            t.minFilter === pc.FILTER_LINEAR_MIPMAP_LINEAR ||
                            t.minFilter === pc.FILTER_NEAREST_MIPMAP_LINEAR ||
                            t.minFilter === pc.FILTER_LINEAR_MIPMAP_NEAREST ||
                            t.minFilter === pc.FILTER_NEAREST_MIPMAP_NEAREST)
                    ) {
    ...
    if (data.hasOwnProperty('bufferView')) {
        ...
        var arrayBuffer = buffers[bufferView.buffer]
        var byteOffset = bufferView.hasOwnProperty('byteOffset')
            ? bufferView.byteOffset
            : 0
        var imageBuffer = arrayBuffer.slice(
            byteOffset,
...
function translateMaterial(data, resources) {
    ...
    if (
        data.hasOwnProperty('extensions') &&
        data.extensions.hasOwnProperty('KHR_materials_unlit')
    ) {
    ...
    if (
        data.hasOwnProperty('extensions') &&
        data.extensions.hasOwnProperty('KHR_materials_pbrSpecularGlossiness')
    ) {
        ...
        if (specData.hasOwnProperty('diffuseTexture')) {
            ...
            if (
                diffuseTexture.hasOwnProperty('extensions') &&
                diffuseTexture.extensions.hasOwnProperty(
                    'KHR_texture_transform'
                )
            ) {
                var diffuseTransformData =
        ...
        if (specData.hasOwnProperty('specularGlossinessTexture')) {
            ...
            material.specularMap =
                resources.textures[specularGlossinessTexture.index]
            ...
            if (
                specularGlossinessTexture.hasOwnProperty('extensions') &&
                specularGlossinessTexture.extensions.hasOwnProperty(
                    'KHR_texture_transform'
                )
            ) {
                var specGlossTransformData =
        ...
        if (pbrData.hasOwnProperty('baseColorTexture')) {
            ...
            if (
                baseColorTexture.hasOwnProperty('extensions') &&
                baseColorTexture.extensions.hasOwnProperty(
                    'KHR_texture_transform'
                )
            ) {
                var baseColorTransformData =
        ...
        if (pbrData.hasOwnProperty('metallicRoughnessTexture')) {
            ...
            material.metalnessMap =
                resources.textures[metallicRoughnessTexture.index]
            ...
            if (
                metallicRoughnessTexture.hasOwnProperty('extensions') &&
                metallicRoughnessTexture.extensions.hasOwnProperty(
                    'KHR_texture_transform'
                )
            ) {
                var metallicTransformData =
    ...
    if (data.hasOwnProperty('normalTexture')) {
        ...
        if (
            normalTexture.hasOwnProperty('extensions') &&
            normalTexture.extensions.hasOwnProperty('KHR_texture_transform')
        ) {
            var normalTransformData =
    ...
    if (data.hasOwnProperty('occlusionTexture')) {
        ...
        if (
            occlusionTexture.hasOwnProperty('extensions') &&
            occlusionTexture.extensions.hasOwnProperty('KHR_texture_transform')
        ) {
            var occlusionTransformData =
    ...
    if (data.hasOwnProperty('emissiveTexture')) {
        ...
        if (
            emissiveTexture.hasOwnProperty('extensions') &&
            emissiveTexture.extensions.hasOwnProperty('KHR_texture_transform')
        ) {
            var emissiveTransformData =
    ...
    data.primitives.forEach(function (primitive) {
            ...
            if (extensions.hasOwnProperty('KHR_draco_mesh_compression')) {
                ...
                switch (geometryType) {
                    ...
                    case decoderModule.POINT_CLOUD:
                        ...
                        status = decoder.DecodeBufferToPointCloud(
                            buffer,
                            outputGeometry
                        )
                        break
                    case decoderModule.TRIANGULAR_MESH:
                        ...
                        status = decoder.DecodeBufferToMesh(
                            buffer,
                            outputGeometry
                        )
                        break
        ...
        if (positions !== null && normals === null) {
            ...
            normals = pc.calculateNormals(
                positions,
                indices === null ? calculateIndices() : indices
            )
        }
        ...
        if (colors !== null) {
            ...
            for (i = 0; i < numVertices; i++) {
                ...
                vertexDataU8[dstIndex] = Math.round(
                    pc.math.clamp(r, 0, 1) * 255
                )
        ...
        if (indices !== null) {
            ...
            var indexBuffer = new pc.IndexBuffer(
                resources.device,
                indexFormat,
                numIndices,
                pc.BUFFER_STATIC,
        ...
        if (primitive.hasOwnProperty('targets')) {
            ...
            primitive.targets.forEach(function (target) {
                ...
                if (target.hasOwnProperty('POSITION')) {
                    ...
                    options.deltaPositions = getAccessorData(
                        gltf,
                        accessor,
                        resources.buffers
                    )
                ...
                if (target.hasOwnProperty('NORMAL')) {
                    ...
                    options.deltaNormals = getAccessorData(
                        gltf,
                        accessor,
                        resources.buffers
                    )
                ...
                if (target.hasOwnProperty('TANGENT')) {
                    ...
                    options.deltaTangents = getAccessorData(
                        gltf,
                        accessor,
                        resources.buffers
                    )
    ...
    gltf.nodes.forEach(function (node, nodeIndex) {
        if (node.hasOwnProperty('mesh')) {
            ...
            meshGroup.forEach(function (mesh) {
                ...
                var meshInstance = new pc.MeshInstance(
                    resources.nodes[nodeIndex],
                    mesh,
                    material
                )
...
export function loadGltf(gltf, device, done, options) {
    var buffers =
        options && options.hasOwnProperty('buffers')
            ? options.buffers
            : undefined
    var basePath =
        options && options.hasOwnProperty('basePath')
            ? options.basePath
            : undefined
    var processUri =
        options && options.hasOwnProperty('processUri')
            ? options.processUri
            : undefined
    var processAnimationExtras =
        options && options.hasOwnProperty('processAnimationExtras')
            ? options.processAnimationExtras
            : undefined
    var processMaterialExtras =
        options && options.hasOwnProperty('processMaterialExtras')
            ? options.processMaterialExtras
            : undefined
    var processGlobalExtras =
        options && options.hasOwnProperty('processGlobalExtras')
            ? options.processGlobalExtras
            : undefined
